...
class Model:
    def __init__(self, config={}, timeframe=2, periods=7200):
        ...
        # Adding MA
        if config.get('MA') != None:
            for i in config.get('MA'):
                stock.MA(i)

        self.dataset = stock.df
        self.X_columns = config['X_columns'] if config.get('X_columns') != None else [
            'Fechamento', 'Volume Quantidade']
        self.y_columns = config['y_columns'] if config.get(
            'y_columns') != None else ['Fechamento']
        self.n_steps = config['n_steps'] if config.get(
            'n_steps') != None else 144
        self.m_predictions = config['m_predictions'] if config.get(
            'm_predictions') != None else 13
        self.test_size = config['test_size'] if config.get(
            'test_size') != None else 0.0
        self.X = None
        self.y = None
        self.scaler_X = None
        self.scaler_y = None
    ...
    def prepare_dataset(self):
        ...
        for i in range(len(X_scaled)):
            ...
            seq_x, seq_y = X_scaled[i:end_ix], y_scaled[end_ix:end_ix +
                                                        self.m_predictions]
    ...
    def fit(self, model_name='default.h5'):
        ...
        layers = (self.config['layers'] if self.config.get(
            'layers') != None else [])
        ...
        for index, layer in enumerate(layers):
            if (index == 0):
                regressor.add(LSTM(units=layer['units'], return_sequences=True, input_shape=(
                    X_train.shape[1], len(self.X_columns))))
            elif (index == len(layers) - 1):
                regressor.add(LSTM(units=layer['units']))
            else:
                regressor.add(
                    LSTM(units=layer['units'], return_sequences=True))
            regressor.add(Dropout(layer['dropout']))

        # Output layer
        regressor.add(Dense(units=self.m_predictions,
                      activation='sigmoid'))  # 8 ou 13
    ...
    def test(self, model_name='default.h5'):
        ...
        y_test = np.reshape(
            self.y_test, (self.y_test.shape[0], self.y_test.shape[1]))
...
with open('config/1.json') as json_file:
    ...
    lstm.fit("2min_MA21.h5")
    lstm.test("2min_MA21.h5")
